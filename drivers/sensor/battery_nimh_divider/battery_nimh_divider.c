 ...
 *
 * ZMK 本体の battery_voltage_divider.c のコピー。
 * lithium_ion_mv_to_pct() は 3.45V 以下を一律 0% にするため、
 * ニッケル水素 2 セル直列 (2.0〜2.8V) では常に 0% になる。
 * %変換だけを NiMH 2S 用の線形マップに差し替えている。
 */

#define DT_DRV_COMPAT cool642tb_battery_nimh_divider
...
static int bvd_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    if (rc == 0) {
        ...
        uint8_t percent = nimh_2s_mv_to_pct(millivolts);
